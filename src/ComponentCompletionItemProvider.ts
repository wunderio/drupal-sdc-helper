...
export class ComponentCompletionItemProvider implements vscode.CompletionItemProvider {
  public async provideCompletionItems(
    ...
    // Matches Twig Include/Embed patterns
    const INCLUDE_EMBED_PATTERN = /(?:\{%|\{\{)\s*(?:include|embed)\s*\(?\s*['"]/;

    if (!linePrefix.match(INCLUDE_EMBED_PATTERN)) {
      return [];
    }

    // Matches the "<sdc_namespace>:" pattern
    const NAMESPACE_PATTERN = /[A-Za-z0-9_-]+:$/;

    const machineNameMatch = linePrefix.match(NAMESPACE_PATTERN);
    if (!machineNameMatch) {
      return [];
    }
    ...
    const matchedComponents = components.filter((component) =>
      component.id.startsWith(machineNamePrefix),
    );
    ...
    return matchedComponents.map((component) => {
      const item = new vscode.CompletionItem(component.id, vscode.CompletionItemKind.File);
